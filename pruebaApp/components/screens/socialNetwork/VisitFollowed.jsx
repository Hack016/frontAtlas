...
export default function VisitFollowed() {
  ...
  const searchUsers = async (initial = false) => {
    ...
    try {
      const response = await fetchWithAuth(
        `${BASE_URL}api/visitFollowed/?username=${encodeURIComponent(username)}&limit=${LIMIT}&offset=${initial ? 0 : offset}`,
        { method: "GET" }
      );
      ...
      {results.length === 0 && !refreshing && !loadingMore ? (
        <View style={styles.container}>
          ...
          <Text style={{ fontSize: 16, color: "gray" }}>
            These are the users that {username} is following
          </Text>
        </View>
      ) : (
        <>
